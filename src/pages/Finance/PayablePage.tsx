...
export default function PayablePage() {
    ...
    const loadData = async () => {
        ...
        try {
            const [list, statistics] = await Promise.all([
                financeService.getTransactions({ tipo: 'despesa', month: new Date() }), // Default to current month for list? Or user choice.
                // Actually service implementation for transactions defaults to ALL if no filter. 
                // Let's load All for list for now, or maybe month filter later.
    ...
    const handlePay = async (t: Transaction) => {
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-10 scroll-smooth">
                <div className="max-w-7xl mx-auto flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between h-32 relative overflow-hidden group">
                            <div className="relative z-10">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <p className="text-gray-500 text-sm font-medium">Vencendo Hoje</p>
                                </div>
                                <p className="text-3xl font-bold text-gray-900 tracking-tight">
                                    R$ {stats?.vencendo_hoje.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                </p>
                                <p className="text-xs text-red-500 mt-1 font-medium">{stats?.vencendo_hoje.count} contas pendentes</p>
                            </div>
                            <div className="absolute right-4 top-4 p-3 bg-red-50 rounded-lg text-red-500 opacity-50 group-hover:opacity-100 transition-opacity">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between h-32 relative overflow-hidden group">
                            <div className="relative z-10">
                                <p className="text-gray-500 text-sm font-medium mb-2">Total a Pagar (Mês)</p>
                                <p className="text-3xl font-bold text-gray-900 tracking-tight">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between h-32 relative overflow-hidden group">
                            <div className="relative z-10">
                                <p className="text-gray-500 text-sm font-medium mb-2">Pago (Mês)</p>
                                <p className="text-3xl font-bold text-gray-900 tracking-tight">
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-100 text-sm">
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={6} className="p-8 text-center text-gray-500">Nenhuma despesa encontrada.</td></tr>
                                    ) : transactions.map(t => (
                                        <tr key={t.id} className="group hover:bg-gray-50 transition-colors">
                                            <td className="p-4">
                                                <p className="font-bold text-gray-900">{t.descricao}</p>
                                                <p className="text-xs text-gray-500">{t.fornecedor_id || '-'}</p>
                                            </td>
                                            <td className="p-4">
